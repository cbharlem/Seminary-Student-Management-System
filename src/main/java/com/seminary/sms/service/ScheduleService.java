...
public class ScheduleService {
    ...
    public List<String> detectConflicts(Schedule candidate) {
        List<String> conflicts = new ArrayList<>();

        List<Schedule> roomConflicts = scheduleRepository.findConflictsByRoom(
            candidate.getRoom().getRoomId(),
            candidate.getDayOfWeek(),
            candidate.getTimeStart(),
            candidate.getTimeEnd());
        if (!roomConflicts.isEmpty()) {
            conflicts.add("Room '" + candidate.getRoom().getRoomName() + "' is already booked at this time.");
        }

        List<Schedule> instrConflicts = scheduleRepository.findConflictsByInstructor(
